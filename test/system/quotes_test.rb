require "application_system_test_case"

class QuotesTest < ApplicationSystemTestCase
  setup do
    @quote = quotes(:one)
  end

  test "visiting the index" do
    visit quotes_url
    assert_selector "h1", text: "Quotes"
  end

  test "creating a Quote" do
    visit quotes_url
    click_on "New Quote"

    fill_in "Amount apartments", with: @quote.amount_apartments
    fill_in "Amount basements", with: @quote.amount_basements
    fill_in "Amount companies", with: @quote.amount_companies
    fill_in "Amount corporations", with: @quote.amount_corporations
    fill_in "Amount elevators", with: @quote.amount_elevators
    fill_in "Amount floors", with: @quote.amount_floors
    fill_in "Amount parking spots", with: @quote.amount_parking_spots
    fill_in "Building type", with: @quote.building_type
    fill_in "Business hours", with: @quote.business_hours
    fill_in "Company name", with: @quote.company_name
    fill_in "Email", with: @quote.email
    fill_in "Full name", with: @quote.full_name
    fill_in "Installation fees", with: @quote.installation_fees
    fill_in "Maximum occupancy", with: @quote.maximum_occupancy
    fill_in "Phone", with: @quote.phone
    fill_in "Product line", with: @quote.product_line
    fill_in "Total cost", with: @quote.total_cost
    click_on "Create Quote"

    assert_text "Quote was successfully created"
    click_on "Back"
  end

  test "updating a Quote" do
    visit quotes_url
    click_on "Edit", match: :first

    fill_in "Amount apartments", with: @quote.amount_apartments
    fill_in "Amount basements", with: @quote.amount_basements
    fill_in "Amount companies", with: @quote.amount_companies
    fill_in "Amount corporations", with: @quote.amount_corporations
    fill_in "Amount elevators", with: @quote.amount_elevators
    fill_in "Amount floors", with: @quote.amount_floors
    fill_in "Amount parking spots", with: @quote.amount_parking_spots
    fill_in "Building type", with: @quote.building_type
    fill_in "Business hours", with: @quote.business_hours
    fill_in "Company name", with: @quote.company_name
    fill_in "Email", with: @quote.email
    fill_in "Full name", with: @quote.full_name
    fill_in "Installation fees", with: @quote.installation_fees
    fill_in "Maximum occupancy", with: @quote.maximum_occupancy
    fill_in "Phone", with: @quote.phone
    fill_in "Product line", with: @quote.product_line
    fill_in "Total cost", with: @quote.total_cost
    click_on "Update Quote"

    assert_text "Quote was successfully updated"
    click_on "Back"
  end

  test "destroying a Quote" do
    visit quotes_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Quote was successfully destroyed"
  end
end
